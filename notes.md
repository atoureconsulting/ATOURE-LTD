...
## 9a. Companies House accounts — confirmed filed with fabricated figures

- **2026-09-17:** User provided the actual filed Companies House PDFs
  (saved to `accounting/companies-house-filings/`): first accounts
  (filed 25 July 2025, for the 13-month period to 30 Sept 2024),
  second accounts (filed 20 June 2026, for period to 30 Sept 2025),
  and the confirmation statement (filed 17 Sept 2026).
- **Confirmation statement is fine** — no financial figures, correctly
  up to date.
- **Both sets of annual accounts contain fabricated/placeholder
  figures, unrelated to the real bank-verified data**, and don't even
  reconcile with each other:
  - First filing: micro-entity (FRS 105) format, total net assets £200.
  - Second filing: uses **FRS 101** instead (wrong basis for a company
    this size — inconsistent with the first filing's FRS 105 basis).
    Its prior-year comparative shows total net assets **£400** — does
    not match the £200 the first filing reported for the same date.
    Shows £3,000 "trade debtors" and £400 "office equipment" that
    don't exist in any bank statement. Profit and loss account balance
    shown as £3,399 vs. real retained profit of ~£113.31 (£33.51 A02 +
    £80.13 Period 3 – wait, actually A01's retained profit was fully
    distributed as dividend, so real combined retained profit at
    30 Sept 2025 is £33.51 + £80.13 = £113.64, matching
    `balance-sheet.md`'s total equity of £114.64 once share capital
    £1.00 is added).
  - Most likely cause: generated by a "quick filing" online tool using
    guessed defaults rather than real figures — not something the user
    did deliberately.
- **Action required:** file amended/replacement accounts for both
  periods with Companies House, using the real bank-verified figures,
  restated into statutory micro-entity (FRS 105) format. This is
  separate from and in addition to the HMRC CT600 filings — different
